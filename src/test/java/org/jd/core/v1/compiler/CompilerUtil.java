...
public class CompilerUtil {
    ...
    public static boolean compile(String preferredJavaVersion, InMemoryJavaSourceFileObject javaFileObject) throws Exception {
        InMemoryClassLoader classLoader = new InMemoryClassLoader();
        return compile(preferredJavaVersion, classLoader, javaFileObject);
    }
    
    public static boolean compile(String preferredJavaVersion, InMemoryClassLoader classLoader, InMemoryJavaSourceFileObject javaFileObject) throws Exception {
        ...
        List<InMemoryJavaSourceFileObject> compilationUnits = Collections.singletonList(javaFileObject);
                            ...
                            default:
                                if (d.getLineNumber() > 0) {
                                    ...
                                    if (!lineNumbers.contains(d.getLineNumber())) {
                                        System.out.println(javaFileObject.getCharContent(true).toString().split("\n")[(int) (d.getLineNumber() - 1)]);
    ...
    public static boolean compile(String preferredJavaVersion, JavaFileObject javaFileObject) throws Exception {
        boolean compilationSuccess = false;
        String javaVersion = getJavaVersion(preferredJavaVersion);
        ...
        try (StandardJavaFileManager standardFileManager = compiler.getStandardFileManager(diagnostics, Locale.US, StandardCharsets.UTF_8)) {
            compilationSuccess = compiler.getTask(writer, standardFileManager, diagnostics, options, null, compilationUnits).call();
